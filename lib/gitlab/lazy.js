...
lazy.show = () => {
  switch(args.show) {
  case 'groups':
    return api.getResource('groups', outputFilters);
  case 'projects':
    return api.getResource('projects', outputFilters);
  case 'group_urls':
    return Groups.getProjectUrls();
  case 'project_urls':
    return Projects.getProjectUrls();
  case 'runners':
    return api.getResource('runners', outputFilters);
  case 'commits':
    return api.getSubResource('projects', 'repository/commits', {
      sortKey: 'created_at',
      attributes: ['title', 'author_name'],
    });
  case 'schedules':
    return api.getSubResource('projects', 'pipeline_schedules', {
      sortKey: 'next_run_at',
      attributes: ['description', 'cron'],
    });
  case 'all_members':
    if (args.groups != undefined) {
      return api.getSubResource('groups', 'members/all', {
	sortKey: 'name',
	...
	.then((members) => {
      ...
      return api.getSubResource('projects', 'members/all', {
	sortKey: 'name',
	attributes: ['name', 'access_level'],
      })
	.then((members) => {
  ...
  default:
    if (args.groups != undefined) {
      return api.getSubResource('groups', args.show, outputFilters);
    } else {
      return api.getSubResource('projects', args.show, outputFilters);
    }
    break;
  }
};
