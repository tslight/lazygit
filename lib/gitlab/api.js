...
api.getResource = (resourceType, out = {}) => {
  return api._get(`/api/v4/${resourceType}`,
	...
	return api._get(`/api/v4/${resourceType}/${id}`);
      });
...
api.getSubResource = (resourceType, subResource, out) => {
  return api.getIds(resourceType)
    .then((ids) => {
      let resources = ids.map((id) => {
	return api._get(`/api/v4/${resourceType}/${id}/${subResource}`);
      });
      return Promise.all((resources))
	.then((resources) => {
	  return util.filter(resources, out);
