...
api._get = (slug, attr = '', limit = Infinity, page = 1, items = []) => {
  let per_page = 100;
  let page_limit = Infinity;
  let last_page_items = per_page;

  if (limit < 100) {
    per_page = limit;
    page_limit = 1;
  } else if (limit >= 100 && limit < Infinity) {
  ...
  return api.get(slug + `?per_page=${per_page}&page=${page}` + attr)
    .then((response) => {
      ...
      if (page >= page_limit) return retrivedItems;
      if (response.headers['x-total'] > retrivedItems.length) {
	page++;
...
//     .then((response) => {
//       let retrivedItems = items.concat(response.data);
//       console.log(`Total: ${response.headers['x-total']}`);
//       if (retrivedItems.length >= response.headers['x-total']) {
//	return retrivedItems;
//       }
//       page++;
//       return api._get(slug, attr, limit, page, retrivedItems);
